...
// Import OpenAI: use new OpenAI() constructor
...
app.post('/conversa', async (req, res) => {
  try {
    ...
    // validate attachments or text
    const hasContent = payload && payload.user && payload.attendant &&
      (payload.message.text || payload.message.caption || payload.message.attachments?.length);
    if (!hasContent) {
    ...
    let textoMensagem = payload.message.text || payload.message.caption || '[attachment]';
    const tipo = payload.message.type || 'text';
    console.log(`[LOG] Nova mensagem recebida de ${nomeCliente}: "${textoMensagem}"`);

    // clean URL
    let url = payload.message.payload?.url?.replace(/;$/, '');
    let contextoExtra = '';
    // transcribe audio
    if (tipo === 'audio' && url) {
    ...
    if (tipo === 'file' && url) {
      try {
        ...
        console.log('[PDF TEXTO]', pdfText.slice(0, 200));
        contextoExtra += '\n' + pdfText;
      } catch (e) {
    ...
    // timing and mapping
    const criadoEm = payload.message.CreatedAt ? new Date(payload.message.CreatedAt) : new Date();
