...
const MENSAGENS = {
  ...
  alerta2: (c, v) => `⏰ *Segundo Alerta - Orçamento em Espera*\n\nPrezada(o) *${v}*, reforçamos que o cliente *${c}* permanece aguardando há 12h úteis.`,
...
async function enviarMensagem(numero, texto) {
  ...
  try {
    ...
    console.error("[ERRO] Falha no envio:", err.message);
  }
}
...
async function transcreverAudio(url) {
  try {
    ...
    console.error("[ERRO] Transcrição falhou:", err.message);
    return null;
  }
}
...
async function extrairTextoPDF(url) {
  try {
    ...
    console.error("[ERRO] PDF inválido:", err.message);
    return null;
  }
}
...
async function isWaitingForQuote(cliente, mensagem, contexto) {
  try {
    const completion = await openai.chat.completions.create({
      model: "gpt-4o",
      messages: [
        { role: "system", content: "Você é um Gerente Comercial IA. Diga apenas 'Sim' se o cliente estiver aguardando orçamento." },
    ...
    console.error("[ERRO] OpenAI falhou:", err.message);
    return false;
  }
}
...
app.post("/conversa", async (req, res) => {
  try {
    ...
    if (!message || !user || !payload.type || payload.type !== "message-received") {
      console.log("[ERRO] Payload incompleto ou evento não suportado:", req.body);
      return res.status(400).json({ error: "Payload incompleto ou tipo não suportado" });
    }
